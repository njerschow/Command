...
final class ClaudeHookServer {
    ...
    private func handleConnection(_ connection: NWConnection) {
        ...
        // Accumulate all data then process
    ...
    /// Find Claude session that matches a TTY (best effort via cwd matching)
    func sessionRequiringAction(forCwd cwd: String) -> ClaudeSession? {
        lock.lock()
        defer { lock.unlock() }
        return sessions.values.first {
            $0.cwd == cwd && ($0.state == .waitingForUser || $0.state == .needsPermission)
        }
    }
